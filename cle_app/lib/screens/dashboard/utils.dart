...
class PaystubApi {
  ...
  static Future<File> generate(PayrollWithItems payinfo, User user) async {
    final pdf = Document();
  ...
  static String genFileName(PayrollWithItems payinfo, User user) {
    String month;
    String day;
    if (payinfo.payroll.periodStart.month < 10) {
      month = '0${payinfo.payroll.periodStart.month}';
    } else {
      month = payinfo.payroll.periodStart.month.toString();
    }
    if (payinfo.payroll.periodStart.day < 10) {
      day = '0${payinfo.payroll.periodStart.month}';
    } else {
      day = payinfo.payroll.periodStart.month.toString();
    }

    return 'paysheet_${user.firstname.toLowerCase()}_${payinfo.payroll.status}_week_of_${payinfo.payroll.periodStart.year}-$month-$day.pdf';
  }
  ...
  static Widget buildTitle(PayrollWithItems payroll) {
    return Column(
            ...
            Column(
              ...
              children: [
                Text('Pay Period',
                    style: TextStyle(fontWeight: FontWeight.bold)),
                Text(
                    '${payroll.payroll.periodStart.month}/${payroll.payroll.periodStart.day}/${payroll.payroll.periodStart.year} - ${payroll.payroll.periodEnd.month}/${payroll.payroll.periodEnd.day}/${payroll.payroll.periodEnd.year}'),
              ],
            ),
          ],
        ),
  ...
  static Widget buildTripsTable(PayrollWithItems payroll) {
    final headers = [
      'Date',
      'Dispatcher',
      'Description',
  ...
  static Widget buildDeductionsTable(PayrollWithItems payroll) {
  ...
  static Widget buildCreditsTable(PayrollWithItems payroll) {
    final headers = ['Date', 'Description', 'Total'];
    //TODO remove test items
  ...
  static Widget buildReimbursmentTable(PayrollWithItems payroll) {
    final headers = ['Date', 'Description', 'Total'];
    //TODO remove test items
    final data = testCredit.map((credit) {
      return [
  ...
  static buildSubTotal(PayrollWithItems payroll) {
    return Row(mainAxisAlignment: MainAxisAlignment.end, children: [
      ...
      Text(
        '\$${payroll.payroll.grossIncome.toStringAsFixed(2)}',
        style: TextStyle(
          fontSize: 16,
          fontWeight: FontWeight.bold,
        ),
  ...
  static buildTotal(PayrollWithItems payroll) {
    return Row(mainAxisAlignment: MainAxisAlignment.end, children: [
      Text(
        'TOTAL CHECK AMOUNT',
        style: TextStyle(
      ...
      SizedBox(width: 2 * PdfPageFormat.cm),
      Text(
        '\$${payroll.payroll.netRevenue.toStringAsFixed(2)}',
        style: TextStyle(
