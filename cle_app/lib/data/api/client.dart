...
abstract class ApiClient extends ChopperService {
  // getpaymenttype
  @Get(path: '/getpaymenttype/{token}')
  Future<Response> getpaymenttype(@Path('token') String token);
  // gettransactions
  @Get(path: '/gettransactions/{token}')
  Future<Response> gettransactions(@Path('token') String token);

  @Post(path: '/addexpensddriver/{token}')
  Future<Response> addexpensddriver(
      @Path('token') String token, @Body() Map<String, dynamic> data);

  @Post(path: '/deleteexpensebyid/{id}/{token}')
  Future<Response> deleteexpensebyid(
      @Path('token') String token, @Path('id') String id);

  @Get(path: '/getallexpencesdriverbyuserid/{id}/{token}')
  Future<Response> getAllExpences(
      @Path('token') String token, @Path('id') int userId);

  @Post(path: '/updateexpense/{token}')
  Future<Response> updateexpense(@Path('token') String token);

  @Get(path: "/getentity/{token}")
  Future<Response> entity(@Path('token') String token);
  @Post(path: "/uploadfilesbase64/{entity}/{idelement}/{token}")
  Future<Response> sendpdffile(@Body() Map<String, dynamic> data,
      @Path('token') String token, @Path('entity') String entity);
  //firbase
  //Ping------------------------------------------------------------------------------------------------
  @Get(path: "/ping")
  ...
  @Get(path: "/getvehiclesbyuser/{id}/{token}")
  Future<Response> getVehiclebyUser(
  ...
  Future<Response> acceptShipmentJob(
      @Path('id') int shipmentId, @Path('token') String token);

  @Put(path: "/pickupthejob/{id}/{token}")
  Future<Response> pickupShipmentJob(@Path('token') String token,
      @Path('id') int shipmentId, @Body() Map<String, dynamic> shipmentData);
  ...
  //Notification------------------------------------------------------------------------------------------------
  @Put(path: "/updateuserfcmtoken/{token}")
  Future<Response> fcmtoken(
      @Path('token') String token, @Body() Map<String, dynamic> fcmInfo);
  @Get(path: "/getallnotificationbyiduser/{token}")
  Future<Response> getallnotication(@Path('token') String token);
  @Delete(path: "/deletenotif/{idnotif}/{token}")
  Future<Response> deleteNotification(
      @Path('idnotif') String id, @Path('token') String token);
  ...
  static ApiClient create() {
    ...
    httpClient.badCertificateCallback =
      ...
      final isValidHost = host == "logistics.tainosystems.com";
    ...
    final client = ChopperClient(
        client: ioClient,
        baseUrl: 'https://logistics.tainosystems.com',
